/**
 * 内存数据源 (MVP): 无 DB 依赖, 便于联调与演示.
 * 说明: id 用 string 兼容多源; coverUrl 必填以统一样式.
 */
...
const DATASET: SearchResult[] = [
    ...
    // XSS test cases: titles and summaries include typical payloads
    {
        id: "13",
...
export default defineEventHandler(async (event) => {
    ...
    const results = DATASET.filter((item) => {
        return item.title.toLowerCase().includes(lowered) || item.summary.toLowerCase().includes(lowered);
    })
        // 限制返回 5 条
        .slice(0, 5);
